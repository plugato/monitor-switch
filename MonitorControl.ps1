<#
  MonitorControl.ps1 - painel completo de controle do monitor via DDC/CI (WinForms).
  Iniciar: MonitorControl.cmd.  Codigos de entrada: config.json.  Acesso DDC/CI: DdcCi.psm1.
  Abas: Entrada e imagem | Cor e modo | Sistema | Avancado.  Todo comando vai para o log da tela e para logs\monitor.log.
#>
...
function Find-Monitor { $m = Get-DdcMonitor; if ($m) { $script:Mon = $m.Handle; return $true }; return $false }
function Read-Mon([int]$code) { DdcCi\Read-Vcp -Handle $script:Mon -Code $code }
function Write-Mon([int]$code, [int]$val, [string]$desc) {
  $ok = DdcCi\Write-Vcp -Handle $script:Mon -Code $code -Value $val
  $st = 'ok'; if (-not $ok) { $st = 'FALHOU' }
  Log ("{0}  0x{1:X2} <- {2} (0x{2:X2})  {3}" -f $desc, $code, $val, $st)
  $ok
}
function Log([string]$s) { $script:txtLog.AppendText(("[{0:HH:mm:ss}] {1}`r`n" -f (Get-Date), $s)) }

# ---------------------------------------------------------------- janela
$form = New-Object Windows.Forms.Form
$form.Text = "$($cfg.monitor) - Controle DDC/CI"
...
$lblStatus.Location = '240,552'; $lblStatus.Size = '375,20'
...
function Add-Combo($parent, $label, $code, $y, $items) {
  ...
  $b = Add-Button $parent 'Aplicar' 510 ($y - 1) 60 26 {
    ...
    if ($i -ge 0) { Write-Mon $this.Tag $c.keys[$i] $c.label | Out-Null }
  }
  $b.Tag = $code
...
Add-Label $tab1 'Fonte de entrada (VCP 0x60 - valores proprietarios Samsung, definidos em config.json)' 10 10 570 $true | Out-Null
$x = 10
foreach ($name in $cfg.inputs.Keys) {
  $b = Add-Button $tab1 ("{0}  (0x{1:X2})" -f $name, [int]$cfg.inputs[$name]) $x 35 180 40 {
    $n = Set-MonitorInput -Source $this.Tag
    Log ("Entrada -> {0}: comando aceito por {1} monitor(es)" -f $this.Tag, $n)
  }
  $b.Tag = $name; $x += 190
}
Add-Label $tab1 'Obs.: se a outra entrada estiver sem sinal, o monitor dorme e so volta pelo botao fisico.' 10 80 560 | Out-Null
...
Add-Button $tab3 'Ligar (1)' 10 35 140 34 { Write-Mon 0xD6 1 'Energia: ligar' | Out-Null } | Out-Null
Add-Button $tab3 'Standby (4)' 160 35 140 34 { if (Confirm 'Colocar o monitor em standby? Ele deve voltar sozinho ao receber sinal, ou pelo botao.') { Write-Mon 0xD6 4 'Energia: standby' | Out-Null } } | Out-Null
Add-Button $tab3 'Desligar (5)' 310 35 140 34 { if (Confirm 'Desligar o monitor? Pode ser necessario ligar pelo botao fisico.') { Write-Mon 0xD6 5 'Energia: desligar' | Out-Null } } | Out-Null

Add-Label $tab3 'Restaurar padroes' 10 95 300 $true | Out-Null
Add-Button $tab3 'Brilho e contraste de fabrica (0x05)' 10 120 260 30 { if (Confirm 'Restaurar brilho e contraste de fabrica?') { Write-Mon 0x05 1 'Restaurar brilho/contraste' | Out-Null; & $script:RefreshAll } } | Out-Null
Add-Button $tab3 'Cores de fabrica (0x08)' 280 120 200 30 { if (Confirm 'Restaurar cores de fabrica?') { Write-Mon 0x08 1 'Restaurar cores' | Out-Null; & $script:RefreshAll } } | Out-Null
Add-Button $tab3 'TUDO de fabrica (0x04)' 10 160 260 30 { if (Confirm 'Restaurar TODAS as configuracoes de fabrica do monitor? Isso apaga seus ajustes.') { Write-Mon 0x04 1 'Restaurar tudo' | Out-Null; & $script:RefreshAll } } | Out-Null
Add-Button $tab3 'Salvar ajustes atuais (0xB0=1)' 280 160 200 30 { Write-Mon 0xB0 1 'Salvar ajustes' | Out-Null } | Out-Null
...
Add-Button $tab4 'Ler' 340 40 80 28 {
  try { $c = [Convert]::ToInt32($script:txtCode.Text, 16); $r = Read-Mon $c
    Log ("Leitura 0x{0:X2}: ok={1} atual={2} (0x{2:X2}) max={3} (0x{3:X2})" -f $c, $r.Ok, $r.Current, $r.Maximum) } catch { Log 'Codigo invalido' }
} | Out-Null
Add-Button $tab4 'Enviar' 430 40 80 28 {
  try { $c = [Convert]::ToInt32($script:txtCode.Text, 16); $v = [Convert]::ToInt32($script:txtVal.Text, 16); Write-Mon $c $v 'Manual' | Out-Null } catch { Log 'Codigo ou valor invalido' }
} | Out-Null
Add-Button $tab4 'Varrer todos os codigos legiveis (0x00-0xFF)' 10 85 300 28 {
  Log 'Varrendo...'; $found = 0
  for ($c = 0; $c -le 0xFF; $c++) { $r = Read-Mon $c; if ($r.Ok) { $found++; Log ("  0x{0:X2}  atual={1,4} (0x{1:X2})  max={2,4} (0x{2:X2})" -f $c, $r.Current, $r.Maximum) } }
  Log "Varredura concluida: $found codigos respondem."
} | Out-Null
Add-Button $tab4 'Mostrar string de capabilities' 320 85 250 28 {
  $caps = Get-DdcCapabilities -Handle $script:Mon
  if ($caps) { Log "CAPS: $caps" } else { Log 'Falha ao ler capabilities' }
} | Out-Null
...
$script:RefreshAll = {
  if ($script:Mon -eq [IntPtr]::Zero -and -not (Find-Monitor)) { $lblStatus.Text = 'Monitor nao encontrado (DDC/CI desligado ou em outra entrada?)'; return }
  if (-not (Read-Mon 0x10).Ok) { $script:Mon = [IntPtr]::Zero; if (-not (Find-Monitor)) { $lblStatus.Text = 'Monitor nao responde'; return } }
  foreach ($code in @($script:Sliders.Keys)) { $r = Read-Mon $code
    if ($r.Ok) { $s = $script:Sliders[$code]; $s.tb.Maximum = [math]::Max(1, $r.Maximum); $s.num.Maximum = $s.tb.Maximum
      $v = [math]::Min($r.Current, $s.tb.Maximum); $s.tb.Value = $v; $s.num.Value = $v } }
  foreach ($code in @($script:Combos.Keys)) { $r = Read-Mon $code
    if ($r.Ok) { $c = $script:Combos[$code]; $c.cb.SelectedIndex = [array]::IndexOf($c.keys, [int]$r.Current) } }
  $info = @()
  ...
  foreach ($k in $map.Keys) { $r = Read-Mon $k; if ($r.Ok) { $info += ("{0,-42} 0x{1:X2} = {2} (0x{2:X}), max {3}" -f $map[$k], $k, $r.Current, $r.Maximum) } }
  $txtInfo.Text = ($info -join "`r`n")
  $lblStatus.Text = ("Atualizado {0:HH:mm:ss} - monitor respondendo" -f (Get-Date))
}
$btnRefresh.Add_Click({ & $script:RefreshAll })

# debounce dos sliders: acumula e envia a cada 200 ms
$timer = New-Object Windows.Forms.Timer; $timer.Interval = 200
$timer.Add_Tick({ if ($script:Pending.Count -gt 0) { $p = $script:Pending; $script:Pending = @{}; foreach ($k in $p.Keys) { Write-Mon $k $p[$k].v $p[$k].d | Out-Null } } })
$timer.Start()
